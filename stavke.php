...
include 'dbh.php';
$clientCheck = $_POST['klijent'];
$query = $sqlP->query("SELECT * FROM clients WHERE Naziv = '" . $clientCheck . "'")->fetch(PDO::FETCH_ASSOC);
//$query = $sqlP->query("SELECT * FROM clients WHERE LIKE N'" . $clientCheck . "'")->fetch(PDO::FETCH_ASSOC);
if ($query == false) {
...
<script>
  $(document).ready(function() {
    ...
    $("#add_bc").focus(function() {
      i++;
      $("#table").prepend('<tr id="row' + i + '"><td>' + (i - 1) + '<td><td><input class="form-control" type="text" placeholder="Unesi barkod" id="barcode" name="barcode[]" value=""></td><td><button type="button" name="remove_bc" class="btn btn-danger remove_bc" id="' + i + '" title="Ukloni"><img src="images/x-icon.svg" alt="x icon"></button><td></tr>');
      $('#barcode').val($("#try").val());
      $('#try').val('').focus();
      $('#first').hide()
    });
