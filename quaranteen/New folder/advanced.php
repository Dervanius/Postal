 ...
 <head>
  ...
  <script>
    $(document).ready(function(){
      ...
      //   var broj = $("#invoice").val();
      //   var brojTrim = broj.replace(/[^a-zA-Z0-9]/g, '');
      //   $("#invoice").val(brojTrim);
      // });
...
<div class="container-fluid">
  ...
  <div class="container mt-5">
    <form action="advanced.php" method="POST">
      <div class="row">
        <div class="col-md-4">
          <div class="form-group mt-3">
            ...
            <input class="form-control" list="clients" name="client" id="client" autocomplete="off" value="<?php if(isset($_POST['submit'])){echo htmlspecialchars($_POST['client']);}?>">
            <datalist id="clients">
            </datalist>
          </div>
        </div>
        <div class="col-md-2">
          <div class="form-group mt-3">
            ...
            <input class="form-control" name="from-date" id="from-date" placeholder="YYYY-MM-DD" autocomplete="off" value="<?php if(isset($_POST['submit'])){echo htmlspecialchars($_POST['from-date']);}?>">
          </div>
        </div>
        <div class="col-md-2">
          <div class="form-group mt-3">
            <label for="to-date">Datum do:</label>
            <input class="form-control" name="to-date" id="to-date" placeholder="YYYY-MM-DD" autocomplete="off" value="<?php if(isset($_POST['submit'])){echo htmlspecialchars($_POST['to-date']);}?>">
          </div>
        </div>
        <div class="col-md-3">
          <div class="form-group mt-3">
            <label for="invoice">Broj dokumenta:</label>
            <input class="form-control" name="invoice" id="invoice" value="<?php if(isset($_POST['submit'])){echo htmlspecialchars(trim($_POST['invoice'], '*()'));}?>">
          </div>
        </div>
      ...
      <div class="row">
        <div class="col-md-2">
            <button type="submit" name="submit" class="btn btn-success mt-3" id="pretraga">Pretraži</button>
        </div>
        <div class="col-md-10">
        </div>
      </div>
  ...
  <div class="container mt-5">
    ...
    if (isset($_POST['submit'])) {
      ...
      $result = $sqlP->query("SELECT * FROM posiljka
        LEFT JOIN clients ON posiljka.KlijentId=clients.EksternaSifra
        INNER JOIN dokument ON dokument.posiljkaId = posiljka.id
        WHERE 1 = 1 ".$getQuery." ORDER BY VremeKreiranja DESC OFFSET  0 ROWS FETCH NEXT 30 ROWS ONLY");
